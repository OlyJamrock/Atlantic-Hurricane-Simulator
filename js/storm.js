...
// Real-world W/P scatter is tighter at the weak end and widens as a
// storm strengthens (a barely-organized TD basically has to sit close
// to its mean pressure; a well-organized Cat3+ can genuinely show more
// variance). Replaces an earlier discrete two-tier clamp — a case-study
// check found even that was too loose (a 55kt storm should never exceed
// 1010mb, an 80kt storm never above 995mb, both meaningfully tighter
// than the old tier boundaries gave). Piecewise-linear between anchors,
// reaching the full flat +-28mb allowance by Cat3 territory.
const MAX_WEAK_OFFSET_ANCHORS = [
  // Offsets derived from stated real-world hard ceilings:
  // 25kt->1020mb, 34kt->1016mb, 50kt->1010mb, 65kt->1000mb,
  // 85kt->990mb, 100kt->970mb, 115kt->960mb (accounts for wind lag).
  // Above 115kt, reverts to the original flat +-28mb allowance exactly
  // (121, not 120, specifically to avoid a tie with 115kt's own
  // ceiling — the mean curve's drop and the offset's jump to 28
  // would otherwise land on the identical value).
  [25, 14.0], [34, 16.0], [50, 17.0], [65, 18.0],
  [85, 25.0], [100, 16.0], [115, 22.0], [121, 28.0],
];
function maxWeakOffsetForWind(windKt) {
  const a = MAX_WEAK_OFFSET_ANCHORS;
  if (windKt <= a[0][0]) return a[0][1];
  if (windKt >= a[a.length - 1][0]) return a[a.length - 1][1];
  for (let i = 0; i < a.length - 1; i++) {
    const [w0, o0] = a[i], [w1, o1] = a[i + 1];
    if (windKt >= w0 && windKt <= w1) return o0 + (o1 - o0) * (windKt - w0) / (w1 - w0);
  }
...
export class Storm {
  ...
  _updatePressure(s, dtDays) {
    ...
    let targetPressureMb = windToPressureMb(this.intensityKt, totalOffsetMb);
    // Hard, real-world-grounded ceiling on how weak a storm's pressure
    // can be for its wind — see maxWeakOffsetForWind above.
    const meanForWind = windToPressureMb(this.intensityKt, 0);
    targetPressureMb = Math.min(targetPressureMb, meanForWind + maxWeakOffsetForWind(this.intensityKt));
  ...
  _troughCapture(env) {
    ...
    if (nearestDist < ENV.troughCaptureRadiusDeg) {
      ...
      const pullMag = proximity * nearestStrength * ENV.troughCaptureMagKtPerStrength;
      // Direction now genuinely depends on where the trough actually is
      // relative to the storm — previously this was a fixed "pull
      // northeast" vector regardless of the trough's real position,
      // which is what was causing "wacky" Caribbean tracks (every
      // trough interaction recurved a storm the same way, no matter
      // its actual geometry) and made "hook into the Gulf" impossible
      // outright (the fixed pull was always eastward). Rotating the
      // direct line-to-trough vector -90 degrees matches real
      // counterclockwise circulation around a Northern Hemisphere upper
      // trough/low: a trough to the northwest pulls a storm northeast
      // (the classic recurve), while a trough positioned to the
      // southeast of the storm pulls it southwest instead — which is
      // the genuine "hook toward the Gulf" case when the geometry lines
      // up that way, not a fixed outcome regardless of position.
      const dirLen = Math.hypot(nearestDLat, nearestDLon) || 1;
      const dirLat = nearestDLat / dirLen, dirLon = nearestDLon / dirLen;
      pullU = dirLat * pullMag;
      pullV = -dirLon * pullMag;
    }
  ...
  step(env, osc, dtDays, dayNum, rand = Math.random) {
    ...
    if (effLand > 0.05) {
      ...
      // waves.js) — this is what actually makes a hyperactive season
      // hyperactive: more storms AND those storms running measurably
      // stronger, rather than just more numerous but otherwise-ordinary
      // ones. Same sign convention as the genesis-frequency modulation:
      // -ENSO (La Nina) and a warm MDR/East-Atlantic anomaly (also
      // covers "warm Canary Current") both boost; +ENSO/a cool MDR
      // anomaly both suppress. Kept modest per-tick since this compounds
      // over a storm's entire lifetime through the relaxation-toward-
      // MPI approach, not just applied once.
      const ensoIdx = osc.ensoIndex(dayNum);
      const seasonIntensityFactor = (1 - ensoIdx * ST.ensoIntensifyCoeff) * (1 + env.mdrEastAtlAnomaly * ST.mdrAnomalyIntensifyCoeff);
      approachRate *= seasonIntensityFactor;
