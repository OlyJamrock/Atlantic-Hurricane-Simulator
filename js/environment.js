  ...
  update(dayNum, activeStorms = [], rand = Math.random) {
    ...
    for (let iLat = 0; iLat < GRID.nLat; iLat++) {
      ...
      for (let iLon = 0; iLon < GRID.nLon; iLon++) {
        ...
        // gaining any northward pull at all (1.0-1.3kt) across the
        // whole approach from the deep MDR to the Bahamas, while the
        // westward push stayed dominant throughout, which is the real
        // cause of waves "continuing west when they should be pulling
        // more WNW/northerly" — real subtropical-ridge periphery flow
        // curves meaningfully as a system approaches the ridge's
        // western flank, not just barely. Boosting the meridional scale
        // specifically (not the shared one) fixes this without also
        ...
        // Real tropical cyclones in the southern MDR/near the ITCZ have
        // much more stable meridional (north-south) motion than routine
        // noise was giving them — this is what was causing storms to
        // "dive south" in the MDR far more often than actually happens.
        // Only tapers the noise-driven V component specifically; the
        // geostrophic/ridge-driven component (geoV, added below,
        // untouched here) is what should occasionally push a storm
        // south at low latitude under genuinely strong ridging — a real
        // but comparatively rare event, not routine variance.
        const southernMeridionalTaper = Math.min(1, Math.max(0.15, (lat - 6) / 10));
        const s850U =
        ...
        const noise850V = this._steerNoiseV.sample(s850U * 1.3, s850V) * ENV.steer850NoiseAmpKt * 0.6 * lowLevelTaper * southernMeridionalTaper;
